...
def getHistogramDirList(hadTau_selection, hadTau_frWeight, hadTau_charge_selection, subcategories):
  histogramDirList = []
  ...
  for cc, cat in enumerate(subcategories) :
      histogramDir = "%s_%s_%s" % (cat, hadTau_charge_selection, hadTau_selection_part1)
      if hadTau_selection_part1.find("Fakeable") != -1:
        if hadTau_frWeight == "enabled":
          histogramDir += "_wFakeRateWeights"
        elif hadTau_frWeight == "disabled":
          histogramDir += "_woFakeRateWeights"
      histogramDirList+=[histogramDir]
  return histogramDirList
...
class analyzeConfig_1l_2tau(analyzeConfig):
  ...
  def __init__(self,
    ...
    self.histogramDir_prep_dcard = histogramDir_prep_dcard_local #"1l_2tau_OS_Tight"
    self.histogramDir_prep_dcard_SS = histogramDir_prep_dcard_SS_local #"1l_2tau_SS_Tight"
  ...
  def createCfg_analyze(self, jobOptions, sample_info, lepton_and_hadTau_selection):
    ...
    jobOptions['histogramDir'] = getHistogramDirList(
      lepton_and_hadTau_selection, lepton_and_hadTau_frWeight, jobOptions['hadTauChargeSelection'], [self.channel]
    )[0]
  ...
  def createCfg_makePlots_mcClosure(self, jobOptions): #TODO
    ...
    for cc, cat_folder in enumerate(self.histogramDir_prep_dcard) :
  ...
  def create(self):
                ...
                for genMatch_category in genMatch_categories:
                  ...
                  if processes_input:
                    ...
                    self.jobOptions_addBackgrounds[key_addBackgrounds_job] = {
                      ...
                      'categories' : getHistogramDirList(lepton_and_hadTau_selection, lepton_and_hadTau_frWeight, hadTau_charge_selection, self.subcategories) ,
    ...
    for histogramToFit in self.histograms_to_fit:
      ...
      for hadTau_charge_selection in self.hadTau_charge_selections:
        ...
        self.jobOptions_add_syst_fakerate[key_add_syst_fakerate_job] = {
          ...
          'category' : self.subcategories, #self.channel,
          'histogramToFit' : histogramToFit,
        ...
        for lepton_and_hadTau_type in [ 'e', 'm', 't' ]:
          ...
          self.jobOptions_add_syst_fakerate[key_add_syst_fakerate_job].update({
            ...
            'histogramName_nominal_%s' % lepton_and_hadTau_type : histogramDir_nominal,
            'inputFile_mcClosure_%s' % lepton_and_hadTau_type : self.jobOptions_addBackgrounds_sum[key_addBackgrounds_job_fakes]['outputFile'],
            'histogramName_mcClosure_%s' % lepton_and_hadTau_type : histogramDir_mcClosure,
          })
    ...
    self.createCfg_makePlots(self.jobOptions_make_plots[key_makePlots_job]) ### Xanda adapt that
    if "SS" in self.hadTau_charge_selections:
