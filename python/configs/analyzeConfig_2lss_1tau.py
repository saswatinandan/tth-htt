...
def getHistogramDirList(lepton_selection, hadTau_selection, lepton_and_hadTau_frWeight, lepton_charge_selection, chargeSumSelection, subcategories):
  histogramDirListL = []
  ...
  for cc, cat in enumerate(subcategories) :
      histogramDir = "%s_lep%s_sum%s_%s" % (cat, lepton_charge_selection, chargeSumSelection, lepton_selection)
...
class analyzeConfig_2lss_1tau(analyzeConfig):
  ...
  def __init__(self,
    ...
    for cc, cat in enumerate(self.subcategories) :
        histogramDir_prep_dcard_local+=[self.subcategories[cc]+"_lepSS_sum%s_Tight"]
  ...
  def createCfg_analyze(self, jobOptions, sample_info, lepton_and_hadTau_selection):
    ...
    histogramDir = getHistogramDirList(
      lepton_and_hadTau_selection, jobOptions['hadTauSelection'], lepton_and_hadTau_frWeight,
      jobOptions['leptonChargeSelection'], jobOptions['chargeSumSelection'], [self.channel]
    )[0]
  ...
  def create(self):
                  ...
                  for genMatch_category in genMatch_categories:
                    ...
                    if processes_input:
                      ...
                      self.jobOptions_addBackgrounds[key_addBackgrounds_job] = {
                        ...
                        'categories' : getHistogramDirList(lepton_selection, hadTau_selection, lepton_and_hadTau_frWeight, lepton_charge_selection, chargeSumSelection, self.subcategories) ,
    ...
    for chargeSumSelection in self.chargeSumSelections:
      histogramDirList_OS = []
      histogramDirList = []
      for hh, hist in enumerate(self.histogramDir_prep_dcard_OS) :
           histogramDirList_OS += [hist % chargeSumSelection]
      ...
      self.jobOptions_addFlips[key_addFlips_job] = {
        ...
        'logFile' : os.path.join(self.dirs[DKEY_LOGS], "addBackgroundLeptonFlips_%s_sum%s.log" % \
          (self.channel, chargeSumSelection)),
        'category_signal' : histogramDirList, #"2lss_1tau_lepSS_sum%s_Tight" % chargeSumSelection,
        'category_sideband' : histogramDirList_OS #"2lss_1tau_lepOS_sum%s_Tight" % chargeSumSelection
      }
    ...
    for chargeSumSelection in self.chargeSumSelections:
      ...
      for histogramToFit in self.histograms_to_fit:
        ...
        #histogramName_nominal_list = [] ### Xanda this needs fixing whe yuo texr where to do the categories list
        ## ==> on a vstring on histogramName or duplicating the PSet's
        ### Xanda: test if this works!!!!
        histogramDir_nominal = []
        histogramDir_mcClosure = []
        if lepton_charge_selection == "OS" :
            for cat in histogramDirList :
            ...
            for cat in histogramDirList_OS :
                histogramDir_nominal+=[ "%s/sel/evt/fakes_mc/%s" % (cat, histogramToFit)]
                histogramDir_mcClosure+=[ "%s/sel/evt/fakes_mc/%s" % (cat.replace("Tight", "Fakeable_mcClosure_wFakeRateWeights"), histogramToFit)]
        for lepton_and_hadTau_type in [ 'e', 'm', 't' ]:
          ...
          self.jobOptions_add_syst_fakerate[key_add_syst_fakerate_job].update({
            ...
            'histogramName_nominal_%s' % lepton_and_hadTau_type : histogramDir_nominal, # "%s/sel/evt/fakes_mc/%s" % (self.histogramDir_prep_dcard % chargeSumSelection, histogramToFit),
            'inputFile_mcClosure_%s' % lepton_and_hadTau_type : self.jobOptions_addBackgrounds_sum[key_addBackgrounds_job_fakes]['outputFile'],
            'histogramName_mcClosure_%s' % lepton_and_hadTau_type : histogramDir_mcClosure #"%s/sel/evt/fakes_mc/%s" % (histogramDir_mcClosure, histogramToFit),
          })
      ...
      for chargeSumSelection in self.chargeSumSelections:
        ...
        if "OS" in self.lepton_charge_selections and chargeSumSelection == "OS":
          ...
          self.createCfg_makePlots(self.jobOptions_make_plots[key_makePlots_job])
        histogramFakesDirList = []
        for hist in histogramDirList :
            histogramFakesDirList+= [hist.replace("Tight", "Fakeable_mcClosure_wFakeRateWeights")]
        if "Fakeable_mcClosure" in self.lepton_and_hadTau_selections and chargeSumSelection == "OS": #TODO
          ...
          self.jobOptions_make_plots[key_makePlots_job] = {
            ...
            'histogramDir_signal' : histogramDirList, #(self.histogramDir_prep_dcard % jobOptions['chargeSumSelection']),
