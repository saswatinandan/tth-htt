...
//--- apply preselection
    ...
    cutFlowTable.update(">= 1 fakeable tau", evtWeight_inclusive);
    cutFlowHistManager->fillHistograms(">= 1 fakeable tau", evtWeight_inclusive);
    const RecoHadTau* fakeableHadTau = fakeableHadTaus[0];
    const hadTauChargeFlipGenMatchEntry& fakeableHadTau_genMatch = getHadTauChargeFlipGenMatch(hadTauGenMatch_definitions, fakeableHadTau);
    int idxFakeableHadTau_genMatch = fakeableHadTau_genMatch.idx_;
    assert(idxFakeableHadTau_genMatch != kGen_HadTauUndefined1);
...
//
    ...
    double mT2_W = -1.;
    Particle::LorentzVector selLeptonP4_lead;
    Particle::LorentzVector selLeptonP4_sublead;
    if (selHadTau->pt() > selLepton->pt())
    {
      selLeptonP4_lead = selHadTau->p4();
      selLeptonP4_sublead = selLepton->p4();
    } else {
      selLeptonP4_lead = selLepton->p4();
      selLeptonP4_sublead = selHadTau->p4();
    }
...
//--- fill histograms with events passing final selection
    ...
    ("met_LD",                                    met_LD)
	  ...
	  ("res-HTT_2",                      max_mvaOutput_HTT_CSVsort4rd_2)
    ...
    ("mvaOutput_1l_1tau_16_variables", mvaOutput_1l_1tau_16_variables)
    //(tH_weight_map)
        .fill();
    }
