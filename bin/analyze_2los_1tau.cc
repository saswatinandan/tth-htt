...
//--- declare missing transverse energy
  ...
  std::vector<std::string> mvaInputVariables_XGB_Updated = {
    "tau_pt", "res-HTT_CSVsort3rd", "avg_dr_jet",
    "tau_eta", "mindr_tau_jet", "HadTop_pt_CSVsort3rd",
...
//--- declare histograms
  ...
  struct selHistManagerType
  {
    ...
    std::map<std::string, EvtHistManager_2los_1tau*> evt_in_categories_in_decayModes_;
    EvtYieldHistManager* evtYield_;
  ...
  for ( std::vector<leptonGenMatchEntry>::const_iterator leptonGenMatch_definition = leptonGenMatch_definitions.begin();
    ...
    for ( std::vector<hadTauGenMatchEntry>::const_iterator hadTauGenMatch_definition = hadTauGenMatch_definitions.begin();
      ...
      vstring categories_evt = {
        "2eos_1tau_btight_0J", "2eos_1tau_bloose_0J",
      ...
      if(isSignal)
      {
        for(const std::string & decayMode_evt: decayModes_evt)
        {
          ...
          for ( vstring::const_iterator category = categories_evt.begin();
            ...
            selHistManager->evt_in_categories_in_decayModes_[category->data()+decayMode_evt] = new EvtHistManager_2los_1tau(makeHistManager_cfg(
              decayMode_and_genMatch,
      ...
      for ( vstring::const_iterator category = categories_evt.begin();
        ...
        TString histogramDir_category = histogramDir.data();
        histogramDir_category.ReplaceAll("2los_1tau", category->data());
        selHistManager->evt_in_categories_[*category] = new EvtHistManager_2los_1tau(makeHistManager_cfg(process_and_genMatch,
  ...
  if ( selectBDT ) {
    ...
    bdt_filler->register_variable<float_type>(
      ...
      "W_pt_semi_boosted_fromAK8", "b_pt_semi_boosted_fromAK8",  //"DR_W_b_gen_AK8",
      "minDR_AK8subjets_lep", "minDR_HTTv2subjets_lep"
    );
    bdt_filler->register_variable<int_type>(
      ...
      "nHTTv2", "nElectron", "nMuon", "AK8_without_subjets",
      //"N_jetAK12",
      "N_jetAK8", "cleanedJets_fromAK8", "hadtruth_semi_boosted_fromAK8",
      ...
      "bWj1Wj2_isGenMatched_semi_boosted", "bWj1Wj2_isGenMatched_semi_boosted_WithKinFit", "bWj1Wj2_isGenMatched_semi_boosted_fromAK8",
...
//--- build collections of jets reconstructed by anti-kT algorithm with dR=1.2 (AK12)
    ...
    //std::vector<const RecoJetAK12*> cleanedJetsAK12 = jetCleanerAK12(jet_ptrsAK12raw, selMuons, selElectrons, selHadTaus);
...
//--- fill histograms with events passing preselection
    ...
    preselHistManager->evt_->fillHistograms(
      ...
      selJets.size(), selBJets_loose.size(), selBJets_medium.size(), sel_HTTv2.size(),
      -1., -1., -1., -1., -1., -1.,
...
for ( std::vector<const RecoJetHTTv2*>::const_iterator jetIter = sel_HTTv2.begin();
  jetIter != sel_HTTv2.end(); ++jetIter ) {
    ...
    if ( bdtResult.at(kXGB_boosted_no_kinFit) > HTT_boosted ) {
     ...
     std::min(
       deltaR(selLepton_lead->p4(), (*jetIter)->subJet3()->p4()),
       deltaR(selLepton_sublead->p4(), (*jetIter)->subJet3()->p4())
     )
     );
    }
    ...
    if ( bdtResult.at(kXGB_boosted_with_kinFit) > HTT_boosted_WithKinFit ) {
      ...
      HadTop_pt_boosted_WithKinFit = (*jetIter)->pt();
  ...
  double minDR_AK12_L = -1.;
  double DR_AK12_tau = -1.;
  /*
  ...
  for ( std::vector<const RecoJet*>::const_iterator selBJet = cleanedJets_fromAK12.begin(); selBJet != cleanedJets_fromAK12.end(); ++selBJet )  { // cleanedJets.size()
    for ( std::vector<const RecoJetAK12*>::const_iterator jetIter = jet_ptrsAK12.begin();
        ...
        if ( bdtResult.at(kXGB_semi_boosted_no_kinFit) > HTT_semi_boosted ) {
          ...
          HadTop_pt_semi_boosted = (*jetIter)->pt();
          ...
          // do the DR qith the selected AK12
          minDR_AK12_L = std::min(
            std::min(
              deltaR(selHadTau->p4(), (*jetIter)->p4()),
              deltaR(selLepton_lead->p4(), (*jetIter)->p4())
            ),
          ...
          DR_AK12_tau = deltaR(selHadTau->p4(), (*jetIter)->p4());
        ...
        if ( bdtResult.at(kXGB_semi_boosted_with_kinFit) > HTT_semi_boosted_WithKinFit ) {
          ...
          HadTop_pt_semi_boosted_WithKinFit = (*jetIter)->pt();
    ...
    */
    ...
    if (genTopPt_CSVsort3rd == genTopPt_boosted)  resolved_and_boosted = true;
...
//--- compute output of BDTs used to discriminate ttH vs. ttbar trained by Kartik for 2los_1tau category
    const std::map<std::string, double>  mvaInputVariables_2los_1tau_evtLevelSUM_TTH_19Var = {
      ...
      { "nJet",          selJets.size()     },
      { "res-HTT_2016",  max_mvaOutput_HTT_2016 },
...
//--- compute output of BDTs used to discriminate ttH vs. ttbar trained by Arun for 2los_1tau category
    ...
    std::string category;
     if ( sel_HTTv2.size() > 0 ) category = "2los_1tau_1pHTTv2"; // "2los_1tau_1pHTTv2", "2los_1tau_1pAK8"
     else if ( jet_ptrsAK8.size() > 0 ) category = "2los_1tau_1pAK8";
     else if ( selElectrons.size() >= 2 && selMuons.size() ==0 && selBJets_medium.size() >= 1  ) category = "2eos_1tau_btight_0J";
     else if ( selElectrons.size() >= 2 && selMuons.size() ==0  ) category = "2eos_1tau_bloose_0J";
     else if ( selMuons.size()     >= 2 && selElectrons.size() == 0 && selBJets_medium.size() >= 1  ) category = "2muos_1tau_btight_0J";
     else if ( selMuons.size()     >= 2 && selElectrons.size() == 0                               ) category = "2muos_1tau_bloose_0J";
     else if ( selMuons.size()     >= 1 && selElectrons.size() >= 1 && selBJets_medium.size() >= 1  ) category = "1mu1eos_1tau_btight_0J";
     else if ( selMuons.size()     >= 1 && selElectrons.size() >= 1                               ) category = "1mu1eos_1tau_bloose_0J";
     else {
...
//--- fill histograms with events passing final selection
    ...
    //std::cout<<" fill histoss 2"<<std::endl;
    selHistManager->evt_->fillHistograms(
      selElectrons.size(), selMuons.size(), selHadTaus.size(),
      selJets.size(), selBJets_loose.size(), selBJets_medium.size(),
      sel_HTTv2.size(),
      mvaOutput_2lss_ttV, mvaOutput_2lss_ttbar, mvaDiscr_2lss, mvaOutput_2los_1tau_ttV,
      mvaOutput_2los_1tau_ttbar, mvaDiscr_2los_1tau,
      mTauTauVis_sel, mvaOutput_2los_1tau_evtLevelSUM_TTH_19Var,
    ...
    if ( bdt_filler ) {
      bdt_filler -> operator()({ eventInfo.run, eventInfo.lumi, eventInfo.event })
          ...
          //("AK12_lead_mass",                jet_ptrsAK12.size() > 0 ? jet_ptrsAK12[0]->p4().mass() : -1 )
          ("minDR_AK12_L",                  minDR_AK12_L)
